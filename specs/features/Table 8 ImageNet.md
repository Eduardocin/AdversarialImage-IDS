...
- The validation split must not include classes or paths assigned to the ImageNet test split.
...
- The `table_8` entry in `configs/experiments.yaml` uses the ImageNet/Caffe
  Table 8 runner, not the MNIST filter-grid runner.
...
- The implementation should reuse existing Table 7 spatial smoothing, ImageNet
  loading, Caffe preprocessing, clean-baseline filtering, and FGSM cache helpers
  where possible.
...
- Running `python scripts/run_experiment.py --experiment table_8` dispatches to
  the ImageNet/Caffe Table 8 flow.
