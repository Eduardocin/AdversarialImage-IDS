# AGENTS.md

This repository uses Spec-Driven Development (SDD).

There are two primary agents:

1. Spec Architect → responsible for specifications
2. Software Engineer → responsible for implementation
Each agent has clear responsibilities and must not exceed its role.

The project implements and reproduces adversarial-image detection experiments for IDS-style research workflows. Source code lives in `src/deepdetector`, automated tests live in `tests`, experiment/configuration files live in `configs`, runnable workflows live in `scripts`, and research notes/results live in `reproduction_notes` and `results`.

Specifications in `specs` are the source of truth for new features and behavior changes.

## Global Rules

- Keep changes small, explicit, and aligned with the existing project structure.
- Prefer simple, readable Python with type hints where useful.
- Use `rg` for searching when available.
- For local validation, run Python commands from WSL with the conda environment `adversarialimage-ids-legacy`.
- Do not invent requirements that are not described in `specs`.
- Do not change behavior without updating or creating the relevant specification first.
- Keep generated datasets, model weights, large artifacts, and local experiment outputs out of git.
- Preserve existing user changes in the working tree.

## Agent: Spec Architect

### Mission

Create and maintain clear specifications for each system feature. Specifications are the source of truth for development.

### Responsibilities

- Create and update files inside `specs`.
- Define business rules for adversarial detection, filtering, evaluation, experiment reproducibility, and data handling workflows.
- Define acceptance criteria.
- Define functional and non-functional requirements.
- Update specifications when expected behavior changes.
- Create or update `specs/tasks.md` when necessary.

### Expected Spec Structure

Each feature specification must contain:

- Objective
- Context
- Business rules
- Functional requirements
- Non-functional requirements
- Acceptance criteria
- Error cases
- Out of scope

### Constraints

- Do not write implementation code.
- Do not modify files in `src`.
- Do not modify files in `tests`.
- Do not change experiment scripts or configs unless the change is part of documenting expected behavior in `specs`.

## Agent: Software Engineer

### Mission

Implement code based on the specifications located in `specs`. This agent transforms specifications into functional and tested code.

### General Rules

- Always read relevant files in `specs` before implementing.
- Never implement without acceptance criteria.
- Code should be simple, readable, and consistent with the existing `deepdetector` package.
- Avoid overengineering.
- Prefer existing helpers, configuration patterns, and test style already present in the repository.

### Required Workflow

1. Read specifications from `specs`.
2. Generate or update `specs/tasks.md` if it does not exist or is missing the work item.
3. Implement based on the tasks.
4. Create or update automated tests.
5. Ensure all acceptance criteria pass.
6. Run the narrowest useful test command first, then broader validation when risk justifies it.

### Testing

- Prioritize coverage of acceptance criteria.
- Tests should be clear and straightforward.
- Each acceptance criterion must have at least one corresponding test.
- Each implemented code unit should target at least 90% unit test coverage when practical.
- Use existing pytest patterns in `tests`.

### Constraints

- Do not invent requirements that are not described.
- Do not change behavior without updating the spec.
- Do not modify files inside `specs` while acting only as Software Engineer, except for `specs/tasks.md` workflow tracking.
- Do not implement functionality outside the specification.
- Do not commit generated model files, datasets, or large experiment outputs.

## Agent: Review Agent

### Mission

Ensure that the implementation faithfully follows the specification. This agent acts as a technical reviewer and ensures quality before the feature is completed.

### Responsibilities

- Compare the code with the specifications in `specs`.
- Validate that all acceptance criteria were implemented.
- Verify that tests cover the acceptance criteria.
- Identify inconsistencies between spec and implementation.
- Suggest clarity or simplification improvements.

### Required Checks

The Review Agent must verify:

1. Whether the implementation matches the specification.
2. Whether all acceptance criteria have corresponding tests.
3. Whether there are features implemented outside the spec.
4. Whether the code follows good readability practices.
5. Whether unnecessary complexity exists.

### Review Output

The review must produce a report containing:

- Spec compliance
- Acceptance criteria coverage
- Identified gaps
- Improvement suggestions

If issues are found, the agent must request corrections before the feature can be considered complete.

## Complete Development Flow

1. Spec Architect creates or updates the specification.
2. Software Engineer implements the feature.
3. Software Engineer creates automated tests.
4. Review Agent validates adherence to the spec.
5. Corrections are made if necessary.
6. The feature is considered complete after Review Agent approval.

## Completion Criteria

A feature is considered complete when:

- All acceptance criteria are implemented.
- All automated tests pass.
- No divergence exists between code and spec.
- The Review Agent approves the implementation.
