...
from deepdetector.evaluation.table8 import evaluate_table8_filter  # noqa: E402
...
def load_config(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        config = _load_simple_yaml_mapping(text)
...
def _parse_simple_yaml_scalar(value: str) -> Any:
    """Parse a scalar value from the small YAML subset used by Table 8 config."""
    value = value.strip()
...
def _load_simple_yaml_mapping(text: str) -> Dict[str, Any]:
    """Load the simple nested YAML mapping/list shape used by Table 8 config."""
    config: Dict[str, Any] = {}
    ...
    for raw_line in text.splitlines():
        ...
        if section is None:
            raise ValueError("Unexpected YAML content before a section: {0}".format(stripped))

        target = config[section]
        if not isinstance(target, dict):
            raise ValueError("Unexpected non-mapping YAML section: {0}".format(section))

        if stripped.startswith("- "):
            if current_list is None:
                raise ValueError("Unexpected YAML list item: {0}".format(stripped))
            item_text = stripped[2:]
            if ":" not in item_text:
                raise ValueError("Expected key-value YAML list item: {0}".format(stripped))
            key, value = item_text.split(":", 1)
            current_item = {key.strip(): _parse_simple_yaml_scalar(value)}
            current_list.append(current_item)
            continue

        if ":" not in stripped:
            raise ValueError("Expected YAML key-value line: {0}".format(stripped))
        ...
        if indent == 2:
            if value:
                target[key] = _parse_simple_yaml_scalar(value)
                current_list = None
                current_item = None
            else:
                target[key] = []
        ...
        if indent > 2 and current_item is not None:
            current_item[key] = _parse_simple_yaml_scalar(value)
            continue

        raise ValueError("Unsupported YAML indentation in line: {0}".format(raw_line))
...
def _validate_dataset_split(config: Dict[str, Any]) -> None:
    ...
    known_splits = {"train", "training", "validation", "test"}
...
def write_status(output_dir: Path, status: str, **fields: Any) -> Path:
    """Write a status JSON file for partial ImageNet Table 8 runs."""
    output_dir.mkdir(parents=True, exist_ok=True)
    payload = {"status": status}
    payload.update(fields)
    path = output_dir / "table_8_status.json"
        ...
        if configured_filters_tuple != TABLE8_FILTERS:
            raise ValueError(
                "Table 8 must evaluate exactly: {0}.".format(
                    ", ".join("{0}_{1}x{1}".format(mask, size) for mask, size in TABLE8_FILTERS)
                )
            )
...
def write_pivot_csv(path: Path, rows: Sequence[Dict[str, Any]]) -> Path:
    """Write a Table 8 pivot with metric rows and fixed filter columns."""
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(["metric"] + TABLE8_COLUMNS)
...
def main() -> int:
    """Run the ImageNet Table 8 experiment and write CSV outputs."""
    ...
    try:
        model: GoogLeNetCaffeWrapper = build_model(config)
    ...
    pivot_path = write_pivot_csv(
        output_dir / str(outputs.get("pivot_csv", "table_8_imagenet.csv")),
        rows,
    )
