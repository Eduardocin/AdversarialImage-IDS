...
DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "experiments" / "imagenet_googlenet_fgsm.yaml"
...
def set_seeds(config: Dict[str, Any]) -> np.random.RandomState:
    """Return a deterministic NumPy RNG from config."""
...
def generate_adversarial_images(
    ...
    """Generate adversarial images with the Caffe FGSM article implementation."""
    attack_config = config.get("attack", {})
    if "epsilon_255" in attack_config:
        epsilon_255 = float(attack_config["epsilon_255"])
    else:
        epsilon_255 = float(attack_config.get("eps", 4.0 / 255.0)) * 255.0
    ...
    if args.adv_path:
        ...
        if adv_images is None:
            write_status(
                ...
                n_loaded=int(len(images)),
                limitation="fgsm_caffe_indisponivel",
                message=(
                    "GoogLeNetCaffeWrapper must expose Caffe gradient support "
                    "or --adv-path must point to compatible adversarial images."
                ),
            )
            return 0
